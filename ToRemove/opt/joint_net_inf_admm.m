...
%     S{3} = S{3}/sum(S{3}(:,1));
    ...
    for signal_trial_idx=1:num_signal_trials
        ...
        for num_signal_idx=1:num_signal_range
            ...
            S_joint = pair_diff(X);
            for k=1:K
                S_joi{graph_trial_idx,signal_trial_idx,num_signal_idx,k} = S_joint{k}/sum(S_joint{k}(:,1));
            end
            fprintf('\n')
            %------------------------
...
error_joi = squeeze(mean(mean(error_joi_trials,1),2));
error_sep = squeeze(mean(mean(error_sep_trials,1),2));

colors = {'#0072BD','#D95319','#EDB120','#7E2F8E','#77AC30'};

figure(1)
semilogx(signal_range,error_joi(:,1),'o-','Color',colors{1},'DisplayName',['Joint ',num2str(1)])
hold on
semilogx(signal_range,error_sep(:,1),'s:','Color',colors{1},'DisplayName',['Separate ',num2str(1)])

semilogx(signal_range,error_joi(:,2),'o-','Color',colors{2},'DisplayName',['Joint ',num2str(2)])
semilogx(signal_range,error_sep(:,2),'s:','Color',colors{2},'DisplayName',['Separate ',num2str(2)])

semilogx(signal_range,error_joi(:,3),'o-','Color',colors{3},'DisplayName',['Joint ',num2str(3)])
semilogx(signal_range,error_sep(:,3),'s:','Color',colors{3},'DisplayName',['Separate ',num2str(3)])
hold off
